...
WizardStyle=modern
SetupIconFile={#RootDir}\assets\app-0.2.12.ico
UninstallDisplayIcon={app}\Server\assets\app-0.2.12.ico
...
Source: "{#RootDir}\assets\app-0.2.12.ico"; DestDir: "{app}\Server\assets"; DestName: "app-0.2.12.ico"; Flags: ignoreversion
...
[Icons]
Name: "{group}\SimplePrint Server"; Filename: "{app}\Server\Gui\SimplePrint.Server.Gui.exe"; IconFilename: "{app}\Server\assets\app-0.2.12.ico"
Name: "{group}\SimplePrint Server deinstallieren"; Filename: "{uninstallexe}"; IconFilename: "{app}\Server\assets\app-0.2.12.ico"
Name: "{commondesktop}\SimplePrint Server"; Filename: "{app}\Server\Gui\SimplePrint.Server.Gui.exe"; IconFilename: "{app}\Server\assets\app-0.2.12.ico"; Tasks: desktopicon
...
Filename: "{sys}\WindowsPowerShell\v1.0\powershell.exe"; Parameters: "-NoProfile -ExecutionPolicy Bypass -Command ""Get-NetFirewallRule -DisplayName 'SimplePrint Discovery' -ErrorAction SilentlyContinue | Remove-NetFirewallRule; Get-NetFirewallRule -DisplayName 'SimplePrint Print Gateway' -ErrorAction SilentlyContinue | Remove-NetFirewallRule; Get-NetFirewallRule -Name 'SimplePrint-PrintShare-*' -ErrorAction SilentlyContinue | Remove-NetFirewallRule; Get-Printer | Where-Object {{ $_.Shared -and ([string]$_.ShareName) -like 'SimplePrint-*' } | ForEach-Object {{ Set-Printer -Name $_.Name -Shared $false -ErrorAction SilentlyContinue }"""; Flags: runhidden waituntilterminated; RunOnceId: RemoveFirewall
