...
Bei Tunnel-Druckern rendert der Client mit exakt dem passenden Treiber und SimplePrint überträgt den fertigen Datenstrom zum Server. Bei Microsoft-IPP/WSD-Druckern versucht SimplePrint zunächst eine echte direkte Windows-Geräteverbindung. Kann der Client das WSD-/IPP-Gerät selbst nicht entdecken, verbindet er automatisch eine vom SimplePrint-Server bereitgestellte Windows-Druckerfreigabe. Dadurch bleibt die physische WSD-/IPP-Verbindung ausschließlich auf dem Server.
...
1. Auf dem Druckserver `SimplePrint-Server-Setup-0.2.10.exe` installieren.
2. `SimplePrint Server` öffnen und die gewünschten lokalen Drucker freigeben.
3. Auf jedem Windows-10/11-Client `SimplePrint-Client-Setup-0.2.10.exe` installieren.
...
```text
dist\Installer\SimplePrint-Server-Setup-0.2.10.exe
dist\Installer\SimplePrint-Client-Setup-0.2.10.exe
```
