...
1. Auf dem Druckserver `SimplePrint-Server-Setup-0.2.5.exe` installieren.
2. `SimplePrint Server` öffnen und die gewünschten lokalen Drucker freigeben.
3. Auf jedem Windows-10/11-Client `SimplePrint-Client-Setup-0.2.5.exe` installieren.
...
7. Microsoft-IPP/WSD-Freigaben werden als **direkte IPP-/WSD-Queue** installiert. Bei WSD akzeptiert SimplePrint ab 0.2.5 neben `DeviceURL` auch `DeviceUUID`, da Windows WSD-Ports die URL nicht auf jedem System über `Get-PrinterPort` zurückliefern.
...
```text
dist\Installer\SimplePrint-Server-Setup-0.2.5.exe
dist\Installer\SimplePrint-Client-Setup-0.2.5.exe
```
